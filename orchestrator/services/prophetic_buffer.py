...
# Stopwords excluded from keyword scoring (module-level for performance).
...
_BRANCH_KEYWORDS: dict[str, frozenset[str]] = {
    ...
    "loot_search": frozenset({
        ...
        "pick", "investigate", "inspect", "scan", "rummage",
        "collect", "retrieve",
    }),
    ...
    "regroup": frozenset({
        "regroup", "gather", "plan", "strategy", "discuss",
    ...
    "call_for_help": frozenset({
        "call", "shout", "yell", "scream", "summon", "signal",
...
class PropheticBuffer:
    ...
    async def get_speculative_response(
        ...
        and ``ambient_audio_key``) if the keyword-match score meets the
    ...
    async def get_prefetched_text(self, intent_id: str) -> str | None:
        """Return the legacy cached text snippet, or None."""
        try:
    ...
    async def get_prefetched_audio(self, intent_id: str) -> str | None:
        """Return the legacy cached ambient audio key, or None."""
        try:
    ...
    async def _prefetch(self, result: "PipelineResult") -> None:
        """Core prefetch: predict branches, gate on load, generate concurrently, cache."""
        if self._settings and not self._settings.speculative_engine_enabled:
            return
        ...
        for label, res in zip(labels_to_run, raw_results):
            if isinstance(res, Exception):
                logger.debug(
                    "PropheticBuffer: branch '%s' generation failed: %s", label, res
                )
                continue
            if res is not None:
                branches.append(res)
    ...
    async def _generate_branch(
        ...
        """
        Generate narrative prose for a single predicted branch.
        Returns None on timeout or generation failure.
        """
    ...
    def _tokenise(text: str) -> frozenset[str]:
        """Lowercase-extract word tokens from text, excluding stopwords."""
    ...
    def _score_branch(cls, player_tokens: frozenset[str], label: str) -> float:
        """
        Precision-based keyword-overlap score: matched / len(player_tokens).

        Answers "what fraction of what the player typed maps to this branch?"
        which is the right question for short, focused player inputs
        (typically 3–8 meaningful tokens).
        """
    ...
    def _effective_branch_count(self) -> int:
        ...
        Gracefully handles any psutil error as a no-op (returns configured max).
        """
